...
class ShopController extends Controller
{
  ...
  {  
    $productId = Cart::where('user_id', Auth::id())->where('product_id' , $product_id)->count();
    $carts =  Cart::where('user_id', Auth::id())->where('product_id',$product_id)->first();
      ...
      if($cartsQty <= $product->product_in_hand_stock)
      {
        ...
        {          
          ...
          else
          {
            ...
            $i = Cart::where('user_id' ,'=', Auth::id())->count();
            return response()->json(['success' => 1 , 'i' => $i]);
          }     
        }  
       else
       {
